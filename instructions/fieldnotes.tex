...
\textbf{Make an inventory of all material you take.} This you give to Andreas before we leave. When we get back we'll check against that to make sure all material is back.
\begin{itemize}
\item Misc:
  \begin{itemize}
  \item several copies of these instructions
  \item Puma-tape and Duck-tape
  \item Sturdy string, about 30m
  ...
  \item one radio (walky-talky, handed out be Andreas)
...
\item Salt dilution:
  ...
  \item salt, pre-packaged (at least for first day 3$\times$ 2kg for proglacial stream, a
    few 100g in various package (20g, 100g, etc) for the glacier streams)
...
Check that:
\begin{itemize}
\item the time of the WTW sensor is set correctly (Swiss summer
time).
\item clear the internal storage
\end{itemize}
If necessary, consult the manual on how to set it.
...
\item open PressureSuite program
\item connect DCX22-CTD, it should then pop up, otherwise "search"
\item "Device configuration" and "Stop recording" (if it is still recording), go back
\item "Read data" and wait for a long time\ldots 20min later it should have finished
\item it now will display as "saved" and we want to export as CSV:
\begin{itemize}
\item "Display"
\item "Export"
\item choose "CSV"
\end{itemize}
% \item \textbf{TODO: update this section. Below is for Logger 5 still:} 
